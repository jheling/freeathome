...
class Crypto():
    ...
    def completeKeyExchange(self,data):
        ...
        if len(data) < (pos + 32):
           raise Exception('KeyExchange response buffer too short, expected 32 bytes for public key at pos' + pos + ",have length " +  len(data)  )
    ...
    def extractData(self, fX, fY):
        ...
        SystemAccessPointResponse = {}
        SystemAccessPointResponse['data'] = ascii
        SystemAccessPointResponse['length'] = 4 + length
    ...
    def decryptPubSub(self, data):
        ...
        if 'update' not in self.__Yq:
            self.__Yq['update'] = {}
            self.__Yq['update']['sequenceCounter'] = cU
        ...
        if nonceNumber < sequenceNumber:
            if nonceNumber not in self.__Yq['update']['skippedSymmetricSequences']:
                raise Exception('Unexpected sequence in received symmetric nonce ' , nonceNumber, '(', sequenceNumber, ')' )
        ...
        pubSubMessage = libnacl.crypto_secretbox_open_easy(data_bytes[libnacl.crypto_box_NONCEBYTES:], nonce, self.__Key)
    ...
    def decryptPayload(self, message):  # data is of type messagereader        
        ...
        if len(messageData) != messageLength:
            raise Exception('Failed to decrypt container: invalid message length ',messageLength, ' expected ',len(messageData)  )    
